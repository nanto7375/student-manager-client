...
export const StudentActionPopup = ({ myLevel, onNavigateDetail, onAddMakeup, onAddFixedMemo, onAddTempMemo }: Props) => (
  <FlexBox flexDirection="column" sx={POPOVER_SX}>
    {myLevel >= 2 && <Button size="small" sx={{ color: 'black' }} onClick={onNavigateDetail}>상세로 이동</Button>}
    {myLevel >= 3 && <Button size="small" sx={{ color: 'black' }} onClick={onAddMakeup}>보강 추가</Button>}
    {myLevel >= 2 && <Button size="small" sx={{ color: 'black' }} onClick={onAddFixedMemo}>고정 메모 추가</Button>}
